...
export default function JumlahKoperasiLainnyaCard({ isLoading, data, tahun, kecamatan }) {
  const theme = useTheme();
  const [jumlahKoperasi, setJumlahKoperasi] = useState(0);
  ...
  useEffect(() => {
    ...
    // hitung total koperasi lainnya
    const total = filtered.reduce((sum, item) => {
      let val = item['Koperasi Lainnya'] ?? 0;
      if (typeof val === 'string') val = val.replace(',', '.').trim();
      const num = parseFloat(val);
      return sum + (isNaN(num) ? 0 : num);
    }, 0);

    setJumlahKoperasi(total);
                ...
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {jumlahKoperasi > 0 ? `${jumlahKoperasi.toLocaleString('id-ID')} Koperasi` : '0 Koperasi'}
                    </Typography>
                  }
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Jumlah Koperasi Lainnya di Kec. {kecamatan}, {tahun}
                      </Typography>
...
JumlahKoperasiLainnyaCard.propTypes = {
