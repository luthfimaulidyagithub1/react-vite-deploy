...
export default function BarPendudukJKDesaCard({ isLoading, data, tahun, kecamatan }) {
  ...
  useEffect(() => {
    ...
    const result = filtered.map((item) => {
      const laki = toNumber(item['penduduk laki']);
      const perempuan = toNumber(item['penduduk perempuan']);
    ...
    <Card sx={{ borderRadius: 2, height: '100%' }}>
      <CardContent sx={{ height: '100%' }}>
        <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 2, color: theme.palette.text.primary, textAlign: 'center' }}>
          Jumlah Penduduk Setiap Desa menurut Jenis Kelamin di Kecamatan {kecamatan}, {tahun}
        </Typography>
          ...
          <ResponsiveContainer width="100%" height={350}>
            <BarChart data={chartData}>
              ...
              <Bar dataKey="laki" fill={theme.palette.primary.main} name="Laki-laki">
                <LabelList dataKey="laki" position="top" fontSize={10} formatter={(val) => val.toLocaleString('id-ID')} />
              </Bar>
              <Bar dataKey="perempuan" fill={theme.palette.error.main} name="Perempuan">
                <LabelList dataKey="perempuan" position="top" fontSize={10} formatter={(val) => val.toLocaleString('id-ID')} />
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        ) : (
...
BarPendudukJKDesaCard.propTypes = {
