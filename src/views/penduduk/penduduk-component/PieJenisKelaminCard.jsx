...
export default function PieJenisKelaminCard({ isLoading, data, tahun, kecamatan }) {
  const [chartData, setChartData] = useState([]);
  const [sumber, setSumber] = useState('');
  const theme = useTheme();

  // Warna khusus: Laki = primary, Perempuan = error
  ...
  useEffect(() => {
    ...
    // hitung total
    const totalLaki = filtered.reduce((sum, item) => sum + toNumber(item['penduduk laki']), 0);
    const totalPerempuan = filtered.reduce((sum, item) => sum + toNumber(item['penduduk perempuan']), 0);
    ...
    >
      <CardContent sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 2, color: theme.palette.text.primary }}>
          Distribusi Penduduk menurut Jenis Kelamin di Kecamatan {kecamatan}, {tahun}
        </Typography>
