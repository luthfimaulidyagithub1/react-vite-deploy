...
export default function StackedAKCard({ isLoading, data, tahun }) {
  ...
  useEffect(() => {
    ...
    filtered.forEach((item) => {
      ...
      if (!grouped[pendidikan]) {
        grouped[pendidikan] = { pendidikan, bekerja: 0, pengangguran: 0 };
      }
      grouped[pendidikan].bekerja += parseInt(item['bekerja'] || 0, 10);
      grouped[pendidikan].pengangguran += parseInt(item['pengangguran'] || 0, 10);
    });
    ...
    >
      <CardContent sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 2, color: theme.palette.text.primary }}>
          Jumlah Angkatan Kerja menurut Pendidikan, {tahun}
        </Typography>
          ...
          {chartData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 10, right: 30, left: 20, bottom: 10 }}>
                ...
                <Bar dataKey="bekerja" stackId="a" fill={theme.palette.success.dark} name="Bekerja" />
                <Bar dataKey="pengangguran" stackId="a" fill={theme.palette.warning.dark} name="Pengangguran Terbuka">
                  {/* Label total di atas stack */}
                  <LabelList
                    dataKey={(entry) => entry.bekerja + entry.pengangguran}
                    position="top"
