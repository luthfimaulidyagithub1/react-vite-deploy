...
export default function PieAKCard({ isLoading, data, tahun }) {
  ...
  useEffect(() => {
    ...
    // filter tahun & kategori Angkatan Kerja
    const filtered = data.filter(
      (item) => String(item.tahun) === String(tahun) && String(item.kategori).toLowerCase() === 'angkatan kerja'
    );
  ...
  // warna khusus
  const colorMap = {
    Bekerja: theme.palette.success.dark,
    'Pengangguran Terbuka': theme.palette.success[200]
  };
  ...
  return (
    ...
    >
      <CardContent sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        {/* Judul */}
        <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 2, color: theme.palette.success.dark }}>
          Distribusi Penduduk Angkatan Kerja Menurut Kegiatan Utama, {tahun}
        </Typography>
