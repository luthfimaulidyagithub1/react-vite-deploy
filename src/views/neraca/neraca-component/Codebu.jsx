...
export default function StackedPDRBCard({ isLoading, data }) {
  ...
  const filteredChartData = chartData.filter((d) => (!tahunAwal || d.tahun >= tahunAwal) && (!tahunAkhir || d.tahun <= tahunAkhir)); // Logika baru untuk memfilter catatan berdasarkan rentang tahun
        ...
        <Box sx={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 2, mb: 2, flexWrap: 'wrap' }}>
          ...
          <FormControl size="small" sx={{ minWidth: 160 }}>
            ...
            <Select labelId="chart-type-label" value={chartType} label="Jenis Grafik" onChange={(e) => setChartType(e.target.value)}>
                            <MenuItem value="area">Stacked Area</MenuItem>              <MenuItem value="line">Line Chart</MenuItem>       
                 {' '}
            </Select>
                     {' '}
          </FormControl>
