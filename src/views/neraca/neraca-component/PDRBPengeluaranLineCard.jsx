...
export default function PDRBPengeluaranLineCard({ isLoading, data }) {
          ...
          <ul style={{ margin: 0, padding: 0, listStyle: 'none' }}>
            {payload.map((entry, idx) => (
              <li key={idx} style={{ color: entry.color }}>
                {entry.name}: {entry.value.toLocaleString()} miliar
              </li>
            ))}
          </ul>
        </div>
              ...
              <ResponsiveContainer width="100%" height={chartHeight}>
                {chartType === 'line' ? (
                  <LineChart data={filteredChartData} margin={{ top: 10, right: 10, left: 0, bottom: 20 }}>
                    <XAxis dataKey="tahun" />
                    <YAxis tickFormatter={(val) => `Rp${val.toLocaleString()}`} width={isMobile ? 70 : 100} />
...
PDRBPengeluaranLineCard.propTypes = {
