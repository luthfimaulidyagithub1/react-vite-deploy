...
export default function LajuPDRBPengeluaranCard({ isLoading, data }) {
  ...
  const chartHeight = isMobile ? 280 : isTablet ? 300 : 400;
  ...
  useEffect(() => {
    ...
    const jenisPengeluaranSet = new Set();
    const catatanMap = {};

    data.forEach((item) => {
      if (item['jenis pengeluaran']) {
        jenisPengeluaranSet.add(String(item['jenis pengeluaran']).trim());
      }
    ...
    const jenisPengeluaranArr = Array.from(jenisPengeluaranSet);
    const newKategoriMap = {};
    const newKategoriList = [];
    const newColorMap = {};
    jenisPengeluaranArr.forEach((jp, index) => {
      const kategori = String.fromCharCode(65 + index); // A, B, C, dst.
      newKategoriList.push(kategori);
      newKategoriMap[kategori] = jp;
      newColorMap[kategori] = colors[index % colors.length];
    });

    setKategoriList(newKategoriList);
    setKategoriMap(newKategoriMap);
    setColorMap(newColorMap);
    if (selectedKategori.length === 0) {
      setSelectedKategori(newKategoriList);
    }

    const grouped = {};
    data.forEach((item) => {
      const th = item.tahun || 'Tidak Diketahui';
      const jp = String(item['jenis pengeluaran']).trim();
      const val = toNumber(item['laju pertumbuhan PDRB ADHK']);

      const kategori = newKategoriList.find((key) => newKategoriMap[key] === jp);

      if (kategori) {
        if (!grouped[th]) {
          grouped[th] = { tahun: th };
        }
        grouped[th][kategori] = val;
      }
    });
      ...
      return (
        <Paper
          ...
          sx={{
            p: 1.5,
            position: 'relative',
            maxWidth: { xs: 200, sm: 500 }, // batas lebar agar wrap di layar kecil
        ...
        >
          <Box
            sx={{
              ...
              alignItems: 'flex-start', // biar bisa wrap kalau panjang
              mb: 1
            }}
          >
            <Typography
            ...
            >
              <strong>Laju pertumbuhan PDRB ADHK {label}</strong>
            </Typography>
            {onClose && (
              <IconButton size="small" onClick={onClose} sx={{ ml: 1, flexShrink: 0 }} aria-label="close">
          ...
          <ul style={{ margin: 0, padding: 0, listStyle: 'none' }}>
            {sortedPayload.map((entry) => (
              ...
              >
                ...
                >
                  {entry.name}. {kategoriMap[entry.name]}:{' '}
    ...
    <Card sx={{ borderRadius: 2, height: '100%' }}>
      <CardContent sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 4, textAlign: 'center', color: theme.palette.orange.dark }}>
          Laju Pertumbuhan Produk Domestik Regional Bruto (PDRB) Atas Dasar Harga Konstan (ADHK) 2010 Menurut Jenis Pengeluaran di Kabupaten
          Sumba Barat
        </Typography>
              ...
              <>
                <ResponsiveContainer width="100%" height={chartHeight}>
                  {chartType === 'line' ? (
                    ...
                    <BarChart data={filteredChartData} onClick={handleChartClick} barCategoryGap="10%" barGap={0}>
                      <XAxis dataKey="tahun" />
                      <YAxis tickFormatter={(val) => `${val.toLocaleString('id-ID', { maximumFractionDigits: 2 })}%`} />
                      <Tooltip content={<CustomTooltip />} trigger="none" />
                      {selectedKategori.map((kat) => (
                        <Bar key={kat} dataKey={kat} fill={colorMap[kat]} name={kategoriMap[kat]} />
                      ))}
                    </BarChart>
                  )}
                </ResponsiveContainer>
