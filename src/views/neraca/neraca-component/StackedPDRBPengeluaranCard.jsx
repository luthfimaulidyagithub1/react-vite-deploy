// StackedPDRBPengeluaranCard.jsx
...
export default function StackedPDRBPengeluaranCard({ isLoading, data }) {
  ...
  useEffect(() => {
    ...
    // ambil list jenis pengeluaran unik
    const pengeluaranSet = new Set();
    data.forEach((item) => {
      if (item['jenis pengeluaran']) {
        pengeluaranSet.add(String(item['jenis pengeluaran']).trim());
      }
    });
    const pengeluaranArr = Array.from(pengeluaranSet);
          ...
          <ul style={{ margin: 0, padding: 0, listStyle: 'none' }}>
            {payload.map((entry, idx) => (
              <li key={idx} style={{ color: entry.color }}>
                {entry.name}: Rp {entry.value?.toLocaleString('id-ID') || 0} miliar
              </li>
            ))}
          </ul>
        </div>
  ...
  const CustomLegend = ({ payload }) => {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', paddingLeft: 20 }}>
        {pengeluaranList.map((jenis, idx) => {
          const color = colors[idx % colors.length];
          return (
            <MuiTooltip key={jenis} title={jenis} arrow placement="left">
              <div style={{ display: 'flex', alignItems: 'center', margin: '4px 0', cursor: 'pointer' }}>
                ...
                <span style={{ fontSize: '0.8rem', color: '#444' }}>{jenis}</span>
              </div>
            </MuiTooltip>
          );
        })}
...
StackedPDRBPengeluaranCard.propTypes = {
