...
export default function JumlahPedagangCard({ isLoading, data, tahun, kecamatan }) {
  const theme = useTheme();
  const [jumlahPedagang, setJumlahPedagang] = useState(0);
  ...
  useEffect(() => {
    ...
    const total = filtered.reduce((sum, item) => {
      let val = item['jumlah pedagang'] ?? 0;
      if (typeof val === 'string') val = val.replace(',', '.').trim();
      const num = parseFloat(val);
      return sum + (isNaN(num) ? 0 : num);
    }, 0);

    setJumlahPedagang(total);
                ...
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {jumlahPedagang > 0 ? `${jumlahPedagang.toLocaleString('id-ID')} Pedagang` : '0 Pedagang'}
                    </Typography>
                  }
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Jumlah Pedagang di Kec. {kecamatan}, {tahun}
                      </Typography>
