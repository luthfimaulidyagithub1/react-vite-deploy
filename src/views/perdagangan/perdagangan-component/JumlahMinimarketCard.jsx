...
export default function JumlahMinimarketCard({ isLoading, data, tahun, kecamatan }) {
  const theme = useTheme();
  const [jumlahMinimarket, setJumlahMinimarket] = useState(0);
  ...
  useEffect(() => {
    ...
    // hitung total minimarket/swalayan/supermarket
    const total = filtered.reduce((sum, item) => {
      let val = item['mini market/swalayan/supermarket'] ?? 0;
      if (typeof val === 'string') val = val.replace(',', '.').trim();
      const num = parseFloat(val);
      return sum + (isNaN(num) ? 0 : num);
    }, 0);

    setJumlahMinimarket(total);
            ...
            <List sx={{ py: 0 }}>
              <ListItem alignItems="center" disableGutters sx={{ py: 0 }}>
                <ListItemAvatar>
                  ...
                  >
                    <StoreMallDirectoryIcon fontSize="inherit" />
                  </Avatar>
                </ListItemAvatar>
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {jumlahMinimarket > 0 ? `${jumlahMinimarket.toLocaleString('id-ID')} Swalayan` : '0 Swalayan'}
                    </Typography>
                  }
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Jumlah Swalayan di Kec. {kecamatan}, {tahun}
                      </Typography>
...
JumlahMinimarketCard.propTypes = {
