// JumlahPasarTanpaBangunanCard.jsx
...
export default function JumlahPasarTanpaBangunanCard({ isLoading, data, tahun, kecamatan }) {
  ...
  useEffect(() => {
    ...
    // hitung total pasar tanpa bangunan
    const total = filtered.reduce((sum, item) => {
      let val = item['pasar tanpa bangunan'] ?? 0;
      if (typeof val === 'string') val = val.replace(',', '.').trim();
      const num = parseFloat(val);
      return sum + (isNaN(num) ? 0 : num);
    }, 0);

    setJumlahPasar(total);
                ...
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {jumlahPasar > 0 ? `${jumlahPasar.toLocaleString('id-ID')} Pasar` : '0 Pasar'}
                    </Typography>
                  }
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Jumlah Pasar Tanpa Bangunan di Kec. {kecamatan}, {tahun}
                      </Typography>
...
JumlahPasarTanpaBangunanCard.propTypes = {
