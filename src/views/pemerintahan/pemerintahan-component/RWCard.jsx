...
export default function RWCard({ isLoading, data, tahun, kecamatan }) {
  const theme = useTheme();
  const [jumlahRW, setJumlahRW] = useState(0);
  const [sumber, setSumber] = useState(null);
  const handleClick = () => {
    setOpen(true);
    setTimeout(() => setOpen(false), 2000); // auto close tooltip after 2 sec
  };
                ...
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {jumlahRW > 0 ? jumlahRW : 'Belum ada data'}
                    </Typography>
                  }
                  secondary={
                    <>
