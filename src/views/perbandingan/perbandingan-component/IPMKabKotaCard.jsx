...
export default function IPMKabkotCard({ isLoading, data }) {
  ...
  useEffect(() => {
    ...
    filteredData.forEach((item) => {
      ...
      const id = String(item.id).trim();
      const val = toNumber(item['IPM']); // ✅ Ganti field ke IPM
  ...
  const HeatmapChart = ({ data, idList }) => {
            ...
            {filteredChartData.length > 0 ? (
              chartType === 'line' || chartType === 'bar' ? (
                ...
                <Box sx={{ width: '100%', height: chartHeight }}>
                  <HeatmapChart data={filteredChartData} idList={idList} />
                </Box>
              )
            ) : (
