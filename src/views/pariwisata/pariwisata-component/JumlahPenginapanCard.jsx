...
export default function JumlahPenginapanCard({ isLoading, data, tahun, kecamatan }) {
  const theme = useTheme();
  const [jumlahPenginapan, setJumlahPenginapan] = useState(0);
  ...
  useEffect(() => {
    ...
    const total = filtered.reduce((sum, item) => {
      let val = item['penginapan'] ?? 0;
      if (typeof val === 'string') val = val.replace(',', '.').trim();
      const num = parseFloat(val);
      return sum + (isNaN(num) ? 0 : num);
    }, 0);

    setJumlahPenginapan(total);
                ...
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {jumlahPenginapan > 0 ? `${jumlahPenginapan.toLocaleString('id-ID')} Penginapan` : '0 Penginapan'}
                    </Typography>
                  }
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Jumlah Penginapan di Kec. {kecamatan}, {tahun}
                      </Typography>
...
JumlahPenginapanCard.propTypes = {
