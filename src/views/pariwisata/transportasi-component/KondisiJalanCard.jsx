...
export default function KondisiJalanCard({ isLoading, data, tahun, kecamatan }) {
  const theme = useTheme();
  const [kondisiTerluas, setKondisiTerluas] = useState('-');
  ...
  useEffect(() => {
    ...
    // hitung total panjang per kondisi jalan
    const totalPerKondisi = {};
    filtered.forEach((item) => {
      const kondisi = String(item['kondisi jalan'] || '-').trim();
      let val = item['panjang (km)'] ?? 0;
      if (typeof val === 'string') val = val.replace(',', '.').trim();
      const num = parseFloat(val);
      if (!kondisi || isNaN(num)) return;
      totalPerKondisi[kondisi] = (totalPerKondisi[kondisi] || 0) + num;
    });

    // ambil kondisi dengan panjang terbesar
    let maxKondisi = '-';
    let maxPanjang = 0;
    Object.entries(totalPerKondisi).forEach(([kondisi, pjg]) => {
      if (pjg > maxPanjang) {
        maxPanjang = pjg;
        maxKondisi = kondisi;
      }
            ...
            <List sx={{ py: 0 }}>
              <ListItem alignItems="center" disableGutters sx={{ py: 0 }}>
                <ListItemAvatar>
                  ...
                  >
                    <StreetviewIcon fontSize="inherit" />
                  </Avatar>
                </ListItemAvatar>
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {kondisiTerluas !== '-' ? `${kondisiTerluas} (${panjang.toLocaleString('id-ID')} km)` : 'Data belum tersedia'}
                    </Typography>
                  }
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Kondisi Jalan Terluas di Kec. {kecamatan}, {tahun}
                      </Typography>
