...
export default function JumlahMenaraTeleponCard({ isLoading, data, tahun, kecamatan }) {
  const theme = useTheme();
  const [jumlahMenara, setJumlahMenara] = useState(0);
  ...
  useEffect(() => {
    ...
    // hitung total menara telepon seluler
    const total = filtered.reduce((sum, item) => {
      let val = item['jumlah menara telepon seluler'] ?? 0;
      if (typeof val === 'string') val = val.replace(',', '.').trim();
      const num = parseFloat(val);
      return sum + (isNaN(num) ? 0 : num);
    }, 0);

    setJumlahMenara(total);
                ...
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {jumlahMenara > 0 ? `${jumlahMenara.toLocaleString('id-ID')} Menara` : '0 Menara'}
                    </Typography>
                  }
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Jumlah Menara Telepon Seluler di Kec. {kecamatan}, {tahun}
                      </Typography>
...
JumlahMenaraTeleponCard.propTypes = {
