// JumlahOperatorKomunikasiCard.jsx
...
export default function JumlahOperatorKomunikasiCard({ isLoading, data, tahun, kecamatan }) {
  const theme = useTheme();
  const [jumlahOperator, setJumlahOperator] = useState(0);
  ...
  useEffect(() => {
    ...
    // hitung total operator layanan komunikasi telepon seluler
    const total = filtered.reduce((sum, item) => {
      let val = item['jumlah operator layanan komunikasi telepon seluler'] ?? 0;
      if (typeof val === 'string') val = val.replace(',', '.').trim();
      const num = parseFloat(val);
      return sum + (isNaN(num) ? 0 : num);
    }, 0);

    setJumlahOperator(total);
                ...
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {jumlahOperator > 0 ? `${jumlahOperator.toLocaleString('id-ID')} Operator` : '0 Operator'}
                    </Typography>
                  }
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Operator Komunikasi Seluler di Kec. {kecamatan}, {tahun}
                      </Typography>
...
JumlahOperatorKomunikasiCard.propTypes = {
