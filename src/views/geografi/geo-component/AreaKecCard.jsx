...
export default function AreaKecCard({ isLoading, data, tahun, kecamatan }) {
  ...
  useEffect(() => {
    ...
    // pastikan tahun sama tipe (string → number atau sebaliknya)
    ...
    // jumlahkan luas desa
    const luas = filtered.reduce((sum, item) => {
      let val = item['luas desa (km2)'] ?? item['luas Desa (km2)'] ?? item['Luas Desa (km2)'] ?? 0;
                ...
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {totalArea > 0 ? `${totalArea.toLocaleString()} km²` : 'Belum ada data'}
                    </Typography>
                  }
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Luas Kecamatan {kecamatan}
                      </Typography>
