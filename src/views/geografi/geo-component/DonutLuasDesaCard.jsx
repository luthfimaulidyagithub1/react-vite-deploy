...
export default function DonutLuasDesaCard({ isLoading, data, tahun, kecamatan }) {
  ...
  useEffect(() => {
    ...
    const result = filtered.map((item) => ({
      name: item.deskel,
      value: parseFloat(item['persentase terhadap luas kecamatan']) || 0
    }));
    ...
    >
      <CardContent sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 2, color: theme.palette.text.primary }}>
          Persentase Luas Desa terhadap Luas Kecamatan di Kecamatan {kecamatan}, {tahun}
        </Typography>
            ...
            {chartData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={chartData}
                    cx="40%" // geser chart biar ada space buat legend kanan
                    cy="50%"
                    innerRadius={50}
                    outerRadius={100}
                    paddingAngle={0}
