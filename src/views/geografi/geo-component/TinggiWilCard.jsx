...
const CardWrapper = styled(MainCard)(({ theme }) => ({
  ...
  '&:after': {
    ...
    width: 210,
    height: 210,
    background: `linear-gradient(210.04deg, ${theme.palette.orange.dark} -50.94%, rgba(255, 193, 7, 0) 83.49%)`,
    borderRadius: '50%',
    top: -30,
    right: -180
  },
  '&:before': {
    ...
    width: 210,
    height: 210,
    background: `linear-gradient(140.9deg, ${theme.palette.orange.dark} -14.02%, rgba(255, 193, 7, 0) 70.50%)`,
    borderRadius: '50%',
    top: -160,
    right: -130
  }
...
export default function TinggiWilCard({ isLoading, data, tahun, kecamatan }) {
  const theme = useTheme();
  const [tinggiWil, setTinggiWil] = useState(null);
  ...
  useEffect(() => {
    ...
    // ambil nilai unik tinggi wilayah
    const tinggiValues = [...new Set(filtered.map((item) => item['tinggi wilayah (mdpl)']))];
    setTinggiWil(tinggiValues.length > 0 ? tinggiValues[0] : null);
                ...
                <ListItemText
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {tinggiWil !== null ? `${tinggiWil.toLocaleString()} mdpl` : 'Belum ada data'}
                    </Typography>
                  }
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Tinggi Wilayah Kecamatan {kecamatan}
                      </Typography>
