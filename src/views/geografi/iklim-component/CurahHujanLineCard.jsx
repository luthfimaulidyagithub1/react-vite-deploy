// src/ui-component/cards/statistik/CurahHujanLineCard.jsx
...
export default function CurahHujanLineCard({ data = [], tahun, isLoading }) {
  ...
  useEffect(() => {
    ...
    // cari key curah hujan
    const mapped = filtered.map((item) => {
      const keys = Object.keys(item || {});
      const curahKey = keys.find((k) => normalize(k).includes('curahhujan'));
      return {
        bulan: item?.bulan || '',
        curah: curahKey ? parseFloat(item[curahKey]) : null
      };
    });
      ...
      <CardContent>
        {isLoading ? (
          ...
          <>
            ...
            >
              Curah Hujan (mm) per Bulan di Stasiun Umbu Mehang Kunda, {tahun}
            </Typography>
                        ...
                        return (
                          ...
                          >
                            ...
                            {payload.map((p, idx) => (
                              ...
                              >
                                Curah Hujan: {p.value != null ? p.value.toFixed(1) : '-'} mm
                              </Typography>
                            ))}
                          </Box>
                        );
