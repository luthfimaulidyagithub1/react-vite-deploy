// src/ui-component/cards/statistik/AnginRataCard.jsx
...
export default function AnginRataCard({ isLoading, data, tahun }) {
  const theme = useTheme();
  const [anginRata, setAnginRata] = useState(null);
  ...
  useEffect(() => {
    ...
    if (filtered.length > 0) {
      // hitung rata-rata dari field "angin rata (m/sec)"
      const total = filtered.reduce((sum, item) => sum + (parseFloat(item['angin rata (m/sec)']) || 0), 0);
      const avg = total / filtered.length;

      setAnginRata(avg.toFixed(2)); // tampilkan 2 digit desimal
      setSumber(filtered[0].sumber || null); // ambil sumber (asumsi sama)
    }
  }, [data, tahun]);
                ...
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {anginRata ? `${anginRata} m/det` : '-'}
                    </Typography>
                  }
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Kecepatan Angin Rata-Rata, {tahun}
                      </Typography>
                      {/* Tombol sumber */}
