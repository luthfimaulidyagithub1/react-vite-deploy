...
export default function DonutPanenBiofarmakaCard({ isLoading, data, tahun, kecamatan }) {
  ...
  useEffect(() => {
    ...
    // filter kategori Biofarmaka + tahun + kecamatan
    ...
    const total = filtered.reduce((sum, item) => sum + toNumber(item['luas panen (ha)']), 0);

    // kelompokkan berdasarkan jenis tanaman
    let grouped = {};
    filtered.forEach((item) => {
      const tanaman = item['jenis tanaman'] || 'Tidak diketahui';
      const jumlah = toNumber(item['luas panen (ha)']);
    ...
    >
      <CardContent sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 2, color: theme.palette.text.primary, textAlign: 'center' }}>
          Distribusi Luas Panen (Ha) Tanaman Biofarmaka di Kecamatan {kecamatan}, {tahun}
        </Typography>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1 }}>
          <Box sx={{ width: '100%', height: 250, position: 'relative' }}>
            ...
            {(!chartData.length || totalPanen === 0) && (
              ...
              >
                Luas Panen Tanaman Biofarmaka = 0
              </Typography>
            )}
          </Box>
