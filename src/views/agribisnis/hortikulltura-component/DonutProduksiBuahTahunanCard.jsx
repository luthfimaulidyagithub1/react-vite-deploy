...
export default function DonutPanenBuahTahunanCard({ isLoading, data, tahun, kecamatan }) {
  ...
  useEffect(() => {
    ...
    // filter kategori BuahTahunanan Tahunan + tahun + kecamatan
    let filtered = data.filter(
      (item) =>
        String(item.kategori).toLowerCase().trim() === 'buah-buahan tahunan' &&
    ...
    >
      <CardContent sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 2, color: theme.palette.text.primary, textAlign: 'center' }}>
          Distribusi Produksi (Kwintal) Buah-Buahan Tahunan di Kecamatan {kecamatan}, {tahun}
        </Typography>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1 }}>
          <Box sx={{ width: '100%', height: 250, position: 'relative' }}>
            ...
            {(!chartData.length || totalPanen === 0) && (
              ...
              >
                Produksi Buah-Buahan Tahunan = 0
              </Typography>
            )}
          </Box>
...
DonutPanenBuahTahunanCard.propTypes = {
