...
export default function DonutPanenBuahSemusimCard({ isLoading, data, tahun, kecamatan }) {
  ...
  useEffect(() => {
    ...
    // filter kategori BuahSemusiman Semusim + tahun + kecamatan
    let filtered = data.filter(
      (item) =>
        String(item.kategori).toLowerCase().trim() === 'buah-buahan semusim' &&
    ...
    const total = filtered.reduce((sum, item) => sum + toNumber(item['produksi (kw)']), 0);

    // kelompokkan berdasarkan jenis tanaman
    let grouped = {};
    filtered.forEach((item) => {
      const tanaman = item['jenis tanaman'] || 'Tidak diketahui';
      const jumlah = toNumber(item['produksi (kw)']);
    ...
    let lainnya = sorted.slice(10);

    if (lainnya.length > 0) {
      const sumLainnya = lainnya.reduce((sum, item) => sum + item.jumlah, 0);
      top10.push({ name: 'Lainnya', jumlah: sumLainnya });
    }
        ...
        >
          ...
          {hoverIndex === index && (
            <Typography component="span" sx={{ ml: 1, fontSize: '0.65rem', color: 'text.secondary' }}>
              {entry.jumlah.toLocaleString('id-ID')} Kwintal ({entry.value.toFixed(2)}%)
            </Typography>
          )}
        </Box>
      ))}
    ...
    >
      <CardContent sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 2, color: theme.palette.text.primary, textAlign: 'center' }}>
          Distribusi Produksi (Kwintal) Buah-Buahan Semusim di Kecamatan {kecamatan}, {tahun}
        </Typography>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1 }}>
          <Box sx={{ width: '100%', height: 250, position: 'relative' }}>
              ...
              <PieChart>
                ...
                <Tooltip
                  formatter={(value, name, props) => {
                    if (!props?.payload || !props.payload.jumlah) return ['0 Kwintal', name];
                    const jumlah = props.payload.jumlah;
                    return [`${jumlah.toLocaleString('id-ID')} Kwintal (${value.toFixed(2)}%)`, props.payload.name];
                  }}
            ...
            {(!chartData.length || totalPanen === 0) && (
              ...
              >
                Produksi Buah-Buahan Semusim = 0
              </Typography>
            )}
          </Box>
...
DonutPanenBuahSemusimCard.propTypes = {
