...
const CardWrapper = styled(MainCard)(({ theme }) => ({
  overflow: 'hidden',
  position: 'relative',
  border: `1px solid ${theme.palette.green ? theme.palette.green.main : theme.palette.orange.main}`,
...
export default function LuasPanenPertanianCard({ isLoading, data, tahun, kecamatan }) {
  ...
  useEffect(() => {
    ...
    // filter kategori: Luas Panen (ha)
    let filtered = data.filter((item) => String(item.kategori).toLowerCase().trim() === 'luas panen (ha)');
                  ...
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Luas Panen Tanaman Pangan di Kec. {kecamatan}, {tahun}
                      </Typography>
...
LuasPanenPertanianCard.propTypes = {
