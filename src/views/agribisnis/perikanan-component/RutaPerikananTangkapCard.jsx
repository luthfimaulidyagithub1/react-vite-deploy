...
export default function RutaPerikananTangkapCard({ isLoading, data, tahun, kecamatan }) {
  const theme = useTheme();
  const [jumlahRuta, setJumlahRuta] = useState(0);
  ...
  useEffect(() => {
    ...
    // hitung total jumlah ruta
    const total = filtered.reduce((sum, item) => {
      let val = item['jumlah ruta'] ?? 0;
                ...
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {jumlahRuta > 0 ? `${jumlahRuta.toLocaleString('id-ID')} Ruta` : '0 Ruta'}
                    </Typography>
                  }
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Rumah Tangga Perikanan Tangkap di Kec. {kecamatan}, {tahun}
                      </Typography>
...
RutaPerikananTangkapCard.propTypes = {
