// ProduksiPerikananBudidayaCard.jsx
...
// assets
import SetMealIcon from '@mui/icons-material/SetMeal'; // icon perikanan

// styles
const CardWrapper = styled(MainCard)(({ theme }) => ({
  overflow: 'hidden',
  position: 'relative',
  border: `1px solid ${theme.palette.info ? theme.palette.warning.main : theme.palette.primary.main}`,
...
export default function ProduksiPerikananBudidayaCard({ isLoading, data, tahun, kecamatan }) {
  ...
  useEffect(() => {
    ...
    const total = filtered.reduce((sum, item) => {
      let val = item['produksi (ton)'] ?? 0;
      if (typeof val === 'string') val = val.replace(',', '.').trim();
      const num = parseFloat(val);
      return sum + (isNaN(num) ? 0 : num);
    }, 0);

    setJumlahProduksi(total);
                ...
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {jumlahProduksi > 0
                        ? `${jumlahProduksi.toLocaleString('id-ID', {
                  ...
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Total Produksi Perikanan Budidaya di Kec. {kecamatan}, {tahun}
                      </Typography>
...
ProduksiPerikananBudidayaCard.propTypes = {
