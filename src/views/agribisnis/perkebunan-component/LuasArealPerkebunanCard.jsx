...
export default function LuasArealPerkebunanCard({ isLoading, data, tahun, kecamatan }) {
  const theme = useTheme();
  const [luasAreal, setLuasAreal] = useState(0);
  ...
  useEffect(() => {
    ...
    // filter kategori: Luas Areal (ha)
    let filtered = data.filter((item) => String(item.kategori).toLowerCase().trim() === 'luas areal (ha)');
                ...
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {luasAreal > 0 ? `${luasAreal.toLocaleString('id-ID')} Hektar` : '0 Hektar'}
                    </Typography>
                  }
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Luas Areal Perkebunan di Kec. {kecamatan}, {tahun}
                      </Typography>
...
LuasArealPerkebunanCard.propTypes = {
