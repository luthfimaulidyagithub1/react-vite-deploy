...
export default function JumlahSekolahSwastaCard({ isLoading, data, tahun, kecamatan }) {
  const theme = useTheme();
  const [jumlahSekolahSwasta, setJumlahSekolahSwasta] = useState(0);
  ...
  useEffect(() => {
    ...
    // hitung total sekolah swasta (field "jumlah sekolah swasta")
    const total = filtered.reduce((sum, item) => {
      let val = item['sekolah swasta'] ?? 0;
      if (typeof val === 'string') val = val.replace(',', '.').trim();
      const num = parseInt(val, 10);
      return sum + (isNaN(num) ? 0 : num);
    }, 0);

    setJumlahSekolahSwasta(total);
                ...
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {jumlahSekolahSwasta > 0 ? jumlahSekolahSwasta.toLocaleString('id-ID') : 'Belum ada data'}
                    </Typography>
                  }
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Jumlah Sekolah Swasta di Kec. {kecamatan}
                      </Typography>
...
JumlahSekolahSwastaCard.propTypes = {
