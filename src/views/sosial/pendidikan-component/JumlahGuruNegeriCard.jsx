...
export default function JumlahGuruNegeriCard({ isLoading, data, tahun, kecamatan }) {
  const theme = useTheme();
  const [jumlahGuruNegeri, setJumlahGuruNegeri] = useState(0);
  ...
  useEffect(() => {
    ...
    // hitung total guru negeri (field "jumlah guru negeri")
    const total = filtered.reduce((sum, item) => {
      let val = item['guru negeri'] ?? 0;
      if (typeof val === 'string') val = val.replace(',', '.').trim();
      const num = parseInt(val, 10);
      return sum + (isNaN(num) ? 0 : num);
    }, 0);

    setJumlahGuruNegeri(total);
                ...
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {jumlahGuruNegeri > 0 ? jumlahGuruNegeri.toLocaleString('id-ID') : 'Belum ada data'}
                    </Typography>
                  }
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Jumlah Guru Negeri di Kec. {kecamatan}, {tahun}
                      </Typography>
...
JumlahGuruNegeriCard.propTypes = {
