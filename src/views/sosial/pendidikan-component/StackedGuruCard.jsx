...
export default function StackedGuruCard({ isLoading, data, tahun, kecamatan }) {
  ...
  useEffect(() => {
    ...
    filtered.forEach((item) => {
      ...
      grouped[pendidikan].negeri += parseInt(item['guru negeri'] || 0, 10);
      grouped[pendidikan].swasta += parseInt(item['guru swasta'] || 0, 10);
    });
    ...
    >
      <CardContent sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 2, color: theme.palette.text.primary }}>
          Jumlah Guru Negeri dan Swasta menurut Tingkat Pendidikan di Kecamatan {kecamatan}, {tahun}
        </Typography>
          ...
          {chartData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} layout="vertical" margin={{ top: 10, right: 30, left: 0, bottom: 20 }}>
                ...
                <Bar dataKey="negeri" stackId="a" fill={theme.palette.success.main} name="Guru Negeri" />
                <Bar dataKey="swasta" stackId="a" fill={theme.palette.secondary.main} name="Guru Swasta">
