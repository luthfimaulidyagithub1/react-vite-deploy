...
export default function JumlahGuruCard({ isLoading, data, tahun, kecamatan }) {
  ...
  useEffect(() => {
    ...
    // hitung total guru (field "jumlah")
    const total = filtered.reduce((sum, item) => {
      let val = item['jumlah guru'] ?? 0;
                ...
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {jumlahGuru > 0 ? jumlahGuru.toLocaleString('id-ID') : 'Belum ada data'}
                    </Typography>
                  }
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Jumlah Guru di Kec. {kecamatan}, {tahun}
                      </Typography>
