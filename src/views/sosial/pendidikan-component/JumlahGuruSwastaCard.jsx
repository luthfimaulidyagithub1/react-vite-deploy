...
export default function JumlahGuruSwastaCard({ isLoading, data, tahun, kecamatan }) {
  const theme = useTheme();
  const [jumlahGuruSwasta, setJumlahGuruSwasta] = useState(0);
  ...
  useEffect(() => {
    ...
    // hitung total guru swasta (field "jumlah guru swasta")
    const total = filtered.reduce((sum, item) => {
      let val = item['guru swasta'] ?? 0;
      if (typeof val === 'string') val = val.replace(',', '.').trim();
      const num = parseInt(val, 10);
      return sum + (isNaN(num) ? 0 : num);
    }, 0);

    setJumlahGuruSwasta(total);
                ...
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {jumlahGuruSwasta > 0 ? jumlahGuruSwasta.toLocaleString('id-ID') : 'Belum ada data'}
                    </Typography>
                  }
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Jumlah Guru Swasta di Kec. {kecamatan}, {tahun}
                      </Typography>
...
JumlahGuruSwastaCard.propTypes = {
