...
export default function JumlahSekolahNegeriCard({ isLoading, data, tahun, kecamatan }) {
  const theme = useTheme();
  const [jumlahSekolahNegeri, setJumlahSekolahNegeri] = useState(0);
  ...
  useEffect(() => {
    ...
    // hitung total sekolah negeri (field "jumlah sekolah negeri")
    const total = filtered.reduce((sum, item) => {
      let val = item['sekolah negeri'] ?? 0;
      if (typeof val === 'string') val = val.replace(',', '.').trim();
      const num = parseInt(val, 10);
      return sum + (isNaN(num) ? 0 : num);
    }, 0);

    setJumlahSekolahNegeri(total);
                ...
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {jumlahSekolahNegeri > 0 ? jumlahSekolahNegeri.toLocaleString('id-ID') : 'Belum ada data'}
                    </Typography>
                  }
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Jumlah Sekolah Negeri di Kec. {kecamatan}
                      </Typography>
...
JumlahSekolahNegeriCard.propTypes = {
