...
export default function DonutMuridCard({ isLoading, data, tahun, kecamatan }) {
  ...
  useEffect(() => {
    ...
    const total = filtered.reduce((sum, item) => sum + toNumber(item['jumlah murid']), 0);

    const result = filtered.map((item) => {
      const jumlah = toNumber(item['jumlah murid']);
      return {
    ...
    >
      <CardContent sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 2, color: theme.palette.text.primary, textAlign: 'center' }}>
          Distribusi Murid menurut Tingkat Pendidikan di Kecamatan {kecamatan}, {tahun}
        </Typography>
