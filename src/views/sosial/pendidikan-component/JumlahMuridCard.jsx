...
export default function JumlahMuridCard({ isLoading, data, tahun, kecamatan }) {
  ...
  useEffect(() => {
    ...
    // hitung total murid (field "jumlah")
    const total = filtered.reduce((sum, item) => {
      let val = item['jumlah murid'] ?? 0;
      if (typeof val === 'string') val = val.replace(',', '.').trim();
      const num = parseInt(val, 10);
      return sum + (isNaN(num) ? 0 : num);
    }, 0);

    setJumlahMurid(total);
                ...
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {jumlahMurid > 0 ? jumlahMurid.toLocaleString('id-ID') : 'Belum ada data'}
                    </Typography>
                  }
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Jumlah Murid di Kec. {kecamatan}, {tahun}
                      </Typography>
