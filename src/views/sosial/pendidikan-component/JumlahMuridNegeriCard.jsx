...
export default function JumlahMuridNegeriCard({ isLoading, data, tahun, kecamatan }) {
  const theme = useTheme();
  const [jumlahMuridNegeri, setJumlahMuridNegeri] = useState(0);
  ...
  useEffect(() => {
    ...
    // hitung total murid negeri (field "jumlah murid negeri")
    const total = filtered.reduce((sum, item) => {
      let val = item['murid negeri'] ?? 0;
      if (typeof val === 'string') val = val.replace(',', '.').trim();
      const num = parseInt(val, 10);
      return sum + (isNaN(num) ? 0 : num);
    }, 0);

    setJumlahMuridNegeri(total);
                ...
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {jumlahMuridNegeri > 0 ? jumlahMuridNegeri.toLocaleString('id-ID') : 'Belum ada data'}
                    </Typography>
                  }
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Jumlah Murid Negeri di Kec. {kecamatan}, {tahun}
                      </Typography>
...
JumlahMuridNegeriCard.propTypes = {
