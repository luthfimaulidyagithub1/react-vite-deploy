...
export default function AgamaTerbanyakCard({ isLoading, data, tahun, kecamatan }) {
  const theme = useTheme();
  const [agamaTerbanyak, setAgamaTerbanyak] = useState(null);
  ...
  useEffect(() => {
    ...
    // hitung total per agama
    const agamaMap = {};
    filtered.forEach((item) => {
      let val = item['jumlah penduduk'] ?? 0;
      if (typeof val === 'string') val = val.replace(',', '.').trim();
      const num = parseInt(val, 10);
      if (!isNaN(num)) {
        agamaMap[item.agama] = (agamaMap[item.agama] || 0) + num;
      }
    });

    // cari agama dengan jumlah terbanyak
    let maxAgama = null;
    let maxJumlah = 0;
    Object.entries(agamaMap).forEach(([agama, total]) => {
      if (total > maxJumlah) {
        maxJumlah = total;
        maxAgama = agama;
      }
                ...
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {agamaTerbanyak ? `${agamaTerbanyak}` : 'Belum ada data'}
                    </Typography>
                  }
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Agama terbanyak di Kec. {kecamatan}, {tahun}
                      </Typography>
