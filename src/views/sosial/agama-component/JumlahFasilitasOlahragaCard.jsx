// JumlahFasilitasOlahragaCard.jsx
...
export default function JumlahFasilitasOlahragaCard({ isLoading, data, tahun, kecamatan }) {
  ...
  useEffect(() => {
    ...
    // hitung total desa yang memiliki fasilitas olahraga
    const total = filtered.reduce((sum, item) => {
      let baik = parseInt(item['kondisi baik'] ?? 0, 10);
      let sedang = parseInt(item['kondisi rusak sedang'] ?? 0, 10);
      let parah = parseInt(item['kondisi rusak parah'] ?? 0, 10);

      baik = isNaN(baik) ? 0 : baik;
      sedang = isNaN(sedang) ? 0 : sedang;
      parah = isNaN(parah) ? 0 : parah;

      return sum + baik + sedang + parah;
    }, 0);

    setJumlahFasilitas(total);
                ...
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {jumlahFasilitas > 0 ? jumlahFasilitas.toLocaleString('id-ID') : 'Belum ada data'}
                    </Typography>
                  }
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Desa dengan Fasilitas Olahraga di Kec. {kecamatan}, {tahun}
                      </Typography>
...
JumlahFasilitasOlahragaCard.propTypes = {
