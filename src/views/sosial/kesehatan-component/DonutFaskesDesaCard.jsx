...
export default function DonutFaskesDesaCard({ isLoading, data, tahun, kecamatan }) {
  ...
  useEffect(() => {
    ...
    const total = filtered.reduce((sum, item) => sum + toNumber(item['jumlah desa']), 0);

    const result = filtered.map((item) => {
      const jumlah = toNumber(item['jumlah desa']);
      return {
        name: item['fasilitas kesehatan'],
        jumlah,
    ...
    >
      <CardContent sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 2, color: theme.palette.text.primary, textAlign: 'center' }}>
          Distribusi Jumlah Desa yang Memiliki Fasilitas Kesehatan di Kecamatan {kecamatan}, {tahun}
        </Typography>
