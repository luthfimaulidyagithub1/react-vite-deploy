...
export default function PieKorbanBencanaCard({ isLoading, data, tahun, kecamatan }) {
  ...
  useEffect(() => {
    ...
    filtered.forEach((item) => {
      grouped['Gempa Bumi'] += toNumber(item['korban gempa bumi']);
      grouped['Tanah Longsor'] += toNumber(item['korban tanah longsor']);
      grouped['Banjir'] += toNumber(item['korban banjir']);
      grouped['Kekeringan'] += toNumber(item['korban kekeringan']);
      grouped['Angin Puyuh'] += toNumber(item['korban angin puyuh']);
    });
  ...
  // label di luar pie, warna sama dengan slice
    ...
    >
      <CardContent sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 2, color: theme.palette.text.primary }}>
          Distribusi Korban Bencana Alam di Kecamatan {kecamatan}, {tahun}
        </Typography>
            ...
            {chartData.some((d) => d.value > 0) ? (
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip formatter={(value) => `${value.toLocaleString('id-ID')} Orang`} />
                </PieChart>
              </ResponsiveContainer>
            ) : (
...
PieKorbanBencanaCard.propTypes = {
