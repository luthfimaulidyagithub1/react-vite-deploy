...
export default function PieFasilitasBABCard({ isLoading, data, tahun, kecamatan }) {
  ...
  useEffect(() => {
    ...
    // kelompokkan berdasarkan kategori: Jamban vs Bukan Jamban
    const jamban = filtered.filter((f) => String(f.kategori).toLowerCase() === 'jamban');
    const bukanJamban = filtered.filter((f) => String(f.kategori).toLowerCase() === 'bukan jamban');

    const sumJamban = jamban.reduce((acc, cur) => acc + toNumber(cur['jumlah desa']), 0);
    const sumBukan = bukanJamban.reduce((acc, cur) => acc + toNumber(cur['jumlah desa']), 0);

    const result = [
      { name: 'Jamban', value: sumJamban },
      { name: 'Bukan Jamban', value: sumBukan }
    ];
  ...
  // warna khusus
  const colorMap = {
    Jamban: theme.palette.secondary.main,
    'Bukan Jamban': theme.palette.error.main
  };
    ...
    >
      <CardContent sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 2, color: theme.palette.text.primary }}>
          Distribusi Jumlah Desa menurut Fasilitas BAB di Kecamatan {kecamatan}, {tahun}
        </Typography>
