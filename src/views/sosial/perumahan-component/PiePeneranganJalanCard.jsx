...
export default function PiePeneranganJalanCard({ isLoading, data, tahun, kecamatan }) {
  ...
  useEffect(() => {
    ...
    // kelompokkan berdasarkan "sumber penerangan jalan utama"
    const grouped = {};
    filtered.forEach((item) => {
      const key = String(item['sumber penerangan jalan utama'] || 'Lainnya');
      grouped[key] = (grouped[key] || 0) + toNumber(item['jumlah desa']);
    });
  ...
  // warna khusus
  const colors = [theme.palette.error.dark, theme.palette.error.light, theme.palette.warning.dark];
  const colorMap = {};
  chartData.forEach((d, i) => {
    colorMap[d.name] = colors[i % colors.length];
  });
  ...
  const renderCustomizedLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent, index, value }) => {
    ...
    const fillColor = getTextColor(colorMap[kategori] || theme.palette.error.dark);
    ...
    >
      <CardContent sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 2, color: theme.palette.text.primary }}>
          Distribusi Jumlah Desa menurut Sumber Penerangan Jalan Utama di Kecamatan {kecamatan}, {tahun}
        </Typography>
...
PiePeneranganJalanCard.propTypes = {
