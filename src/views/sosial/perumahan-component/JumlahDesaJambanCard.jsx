...
export default function JumlahDesaJambanCard({ isLoading, data, tahun, kecamatan }) {
  const theme = useTheme();
  const [jumlahDesa, setJumlahDesa] = useState(0);
  ...
  useEffect(() => {
    ...
    // filter kategori jamban
    let filtered = data.filter((item) => String(item.kategori).toLowerCase().trim() === 'jamban');
                ...
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {jumlahDesa > 0 ? jumlahDesa.toLocaleString('id-ID') : 'Belum ada data'}
                    </Typography>
                  }
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Desa yang Memiliki Jamban di Kec. {kecamatan}, {tahun}
                      </Typography>
...
JumlahDesaJambanCard.propTypes = {
