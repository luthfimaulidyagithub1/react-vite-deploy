...
export default function PieJambanCard({ isLoading, data, tahun, kecamatan }) {
  ...
  useEffect(() => {
    ...
    const filtered = data.filter(
      (item) =>
        ...
        String(item.kategori).toLowerCase() === 'jamban'
    );
    ...
    // kelompokkan berdasarkan fasilitas BAB
    const grouped = {};
    filtered.forEach((item) => {
      const key = item['fasilitas BAB'] || 'Tidak Diketahui';
    ...
    >
      <CardContent sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 2, color: theme.palette.text.primary }}>
          Distribusi Jumlah Desa menurut Kepemilikan Jamban di Kecamatan {kecamatan}, {tahun}
        </Typography>
