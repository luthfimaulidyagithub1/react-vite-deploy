...
const CardWrapper = styled(MainCard)(({ theme }) => ({
  ...
  '&:after': {
    ...
    width: 210,
    height: 210,
    background: `linear-gradient(210.04deg, ${theme.palette.orange.dark} -50.94%, rgba(255, 152, 0, 0) 83.49%)`,
    borderRadius: '50%',
    top: -30,
    right: -180
  },
  '&:before': {
    ...
    width: 210,
    height: 210,
    background: `linear-gradient(140.9deg, ${theme.palette.orange.dark} -14.02%, rgba(255, 152, 0, 0) 70.50%)`,
    borderRadius: '50%',
    top: -160,
    right: -130
  }
...
export default function JumlahBencanaLongsorCard({ isLoading, data, tahun, kecamatan }) {
  const theme = useTheme();
  const [jumlahGempa, setJumlahGempa] = useState(0);
  ...
  useEffect(() => {
    ...
    // hitung total dari field "tanah longsor"
    const total = filtered.reduce((sum, item) => {
      let val = item['tanah longsor'] ?? 0;
      if (typeof val === 'string') val = val.replace(',', '.').trim();
      const num = parseInt(val, 10);
      return sum + (isNaN(num) ? 0 : num);
    }, 0);

    setJumlahGempa(total);
                ...
                <ListItemText
                  ...
                  primary={
                    <Typography variant="h4" sx={{ color: '#000' }}>
                      {jumlahGempa > 0 ? jumlahGempa.toLocaleString('id-ID') : '0'}
                    </Typography>
                  }
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Jumlah Tanah Longsor di Kec. {kecamatan}, {tahun}
                      </Typography>
...
JumlahBencanaLongsorCard.propTypes = {
