...
const CardWrapper = styled(MainCard)(({ theme }) => ({
  ...
  '&:after': {
    ...
    width: 210,
    height: 210,
    background: `linear-gradient(210.04deg, ${theme.palette.primary.dark} -50.94%, rgba(255, 152, 0, 0) 83.49%)`,
    borderRadius: '50%',
    top: -30,
    right: -180
  },
  '&:before': {
    ...
    width: 210,
    height: 210,
    background: `linear-gradient(140.9deg, ${theme.palette.primary.dark} -14.02%, rgba(255, 152, 0, 0) 70.50%)`,
    borderRadius: '50%',
    top: -160,
    right: -130
  }
...
export default function JumlahBencanaGempaCard({ isLoading, data, tahun, kecamatan }) {
  ...
  useEffect(() => {
    ...
    // hitung total dari field "gempa bumi"
    const total = filtered.reduce((sum, item) => {
      let val = item['gempa bumi'] ?? 0;
                  ...
                  secondary={
                    <>
                      <Typography variant="subtitle2" sx={{ color: '#000', mt: 0.25 }}>
                        Jumlah Gempa Bumi di Kec. {kecamatan}, {tahun}
                      </Typography>
...
JumlahBencanaGempaCard.propTypes = {
