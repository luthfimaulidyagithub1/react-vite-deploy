...
export default function PiePelangganListrikCard({ isLoading, data, tahun }) {
  ...
  useEffect(() => {
    ...
    filtered.forEach((item) => {
      ...
      const jumlah = toNumber(item['jumlah pelanggan listrik']);
  ...
  const renderCustomizedLabel = ({ cx, cy, midAngle, outerRadius, percent, index, value }) => {
    ...
    return (
      ...
      >
        {(percent * 100).toLocaleString('id-ID', {
    ...
    >
      <CardContent sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 2, color: theme.palette.text.primary, textAlign: 'center' }}>
          Distribusi Pelanggan Listrik menurut Kecamatan, {tahun}
        </Typography>
...
PiePelangganListrikCard.propTypes = {
