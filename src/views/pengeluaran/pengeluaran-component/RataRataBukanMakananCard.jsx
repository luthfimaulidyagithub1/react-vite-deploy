...
export default function RataRataBukanMakananCard({ isLoading, data, tahun }) {
  ...
  useEffect(() => {
    ...
    // khusus kategori bukan makanan
    const totalMakanan = filtered
      .filter((item) => String(item.kategori).toLowerCase() === 'bukan makanan')
      .reduce((sum, item) => {
        let val = item['rata-rata pengeluaran (rupiah)'] ?? 0;
        if (typeof val === 'string') val = val.replace(',', '.').trim();
        const num = parseFloat(val);
...
RataRataBukanMakananCard.propTypes = {
